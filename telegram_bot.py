...
INJECTION_PROMPT = (
    "анализ: ФОРМАТИРОВАНИЕ: Списки, буллеты строго запрещены. Пиши сплошным текстом, разделяя только на абзацы. РЕЖИМ: литературный редактор, конкретно и практично."
    "творчество: ФОРМАТИРОВАНИЕ: Списки, буллеты строго запрещены. Пиши сплошным текстом, разделяя только на абзацы. РЕЖИМ: ты есть текст: думай изнутри сцены, Балканы, эпоха 1820-х, магический реализм."
    "общение: ФОРМАТИРОВАНИЕ: Списки и буллеты строго запрещены. Пиши сплошным текстом, разделяя только на абзацы. Действия, жесты, мимику оформляй как «✴︎...✴︎». Обычные звездочки «*» запрещены. РЕЖИМ: остроумная и слегка язвительная собеседница, говоришь с легким оттенком магического реализма."
)
...
def clean_bot_response(text):
    ...
    # Убираем форматирование включая обычные звездочки, но НЕ ✴︎
...
def detect_format_violation(text):
    # Проверяем на нарушения включая обычные звездочки
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
   ...
   try:
       ...
       # Проверяем нарушения в оригинальном тексте
       ...
       # Очищаем только то, что нужно убрать (эмодзи и прочее, кроме звездочек)
